...
async function main() {
  ...
  try {
    ...
    browser = await chromium.connectOverCDP(SBR_WS, { timeout: 60000 });
    log('Connected');
    ...
    for (let i = 0; i < validRestaurants.length; i++) {
      ...
      if (!hasRouter) {
        ...
        try { await page.goto(HOMEPAGE_URL, { waitUntil: 'networkidle', timeout: HOMEPAGE_TIMEOUT }); } catch {}
        await page.waitForTimeout(HOMEPAGE_WAIT);
      }
