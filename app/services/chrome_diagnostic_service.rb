class ChromeDiagnosticService
  ...
  def self.get_version(binary_path)
    return nil unless File.exist?(binary_path) && File.executable?(binary_path)
    
    system(binary_path, "--version", out: "/tmp/chrome_version.txt", err: "/dev/null")
    File.read("/tmp/chrome_version.txt").strip if File.exist?("/tmp/chrome_version.txt")
  rescue
    nil
  ensure
    File.delete("/tmp/chrome_version.txt") if File.exist?("/tmp/chrome_version.txt")
  end

  def self.get_chromedriver_version(binary_path)
    return nil unless File.exist?(binary_path) && File.executable?(binary_path)
    
    system(binary_path, "--version", out: "/tmp/chromedriver_version.txt", err: "/dev/null")
    File.read("/tmp/chromedriver_version.txt").strip if File.exist?("/tmp/chromedriver_version.txt")
  rescue
    nil
  ensure
    File.delete("/tmp/chromedriver_version.txt") if File.exist?("/tmp/chromedriver_version.txt")
  end
end
