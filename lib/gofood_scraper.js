...
async function main() {
  ...
  try {
    ...
    // Block heavy resources to reduce proxy traffic
    // We only need HTML + JS (for Next.js hydration) + _next/data JSON
    await page.route('**/*', route => {
      const url = route.request().url();
      const type = route.request().resourceType();
      if (type === 'document' || url.includes('_next/data')) return route.continue();
      if (type === 'script' && url.includes('_next/static')) return route.continue();
      if (['image', 'media', 'font', 'stylesheet'].includes(type)) return route.abort();
      if (url.includes('googletagmanager') || url.includes('golabs.io') || url.includes('onetrust') ||
          url.includes('raccoon') || url.includes('gstatic') || url.includes('deals/profile')) return route.abort();
